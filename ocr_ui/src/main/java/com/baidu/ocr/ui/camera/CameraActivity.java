...
public class CameraActivity extends Activity {
    ...
    private String camerFilePath;//修改
    private boolean isNextPage;//是否跳转到下一页
    @Override
...
//        String outputPath = getIntent().getStringExtra(KEY_OUTPUT_FILE_PATH);
        String outputPath = new File(getFilesDir(), "ocrPic.jpg").getAbsolutePath();
        ...
        isNextPage=getIntent().getBooleanExtra("isNative",true);
        if (isNextPage) {
            Log.i("TAG",true+"");
        }else {
            Log.i("TAG",false+"");
...
//             outputFile = new File(this.getFilesDir(), "ocrPic.jpg");//保存file
        }
    ...
    private void doConfirmResult() {
        CameraThreadPool.execute(new Runnable() {
            ...
            public void run() {
                ...
                if (isNextPage) {
                    ComponentName componentName=new ComponentName("com.bibinet.intelibuild","com.bibinet.intelibuild.project.ui.activity.OcrIndentifyResultActivity");
                    Intent intent1=new Intent();
                    intent1.putExtra("camerFilePath",outputFile.getAbsolutePath());
